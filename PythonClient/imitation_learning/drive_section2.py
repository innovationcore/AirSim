...
while running == True:    
    ...
    if abs(car_controls.steering) <= 1.0:
        car_controls.throttle = 0.6-(0.4*abs(car_controls.steering))
    else:
    ...
    if (position.x_val >= 100) & (position.x_val <= 105):
        if (position.y_val >= -10) & (position.y_val <= -2):
            running = False
...
exec(open("drive_section3.py").read())
